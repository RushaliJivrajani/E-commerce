    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading discount vouchers...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-3xl flex items-center gap-2">
            Discount Coupons & Vouchers <Ticket className="h-6 w-6 text-slate-500" />
          </h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-500 dark:text-slate-400">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {coupons.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-extrabold text-slate-500 dark:text-slate-400 text-sm tracking-wider font-mono bg-slate-500/10 px-2.5 py-1 rounded-lg">
                      {c.code}
                    </span>
                  </td>

                  <td className="px-6 py-4 text-slate-800 dark:text-slate-300 font-medium">
                    {c.type}
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className="text-slate-600 dark:text-slate-300 font-bold">{c.usageCount}</span>
                    <span className="text-slate-400"> / {c.usageLimit} limit</span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => handleOpenEdit(c)} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-850"><Edit className="h-4 w-4" /></button>
                      <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-rose-500/10"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
              ...
              {coupons.length === 0 && (
                <tr>
                  <td colSpan={8} className="text-center py-12 text-slate-400">No discount codes created yet.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="relative w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Promo Code Symbol</label>
                  <input
                    type="text"
                    required
                    value={formCode}
                    ...
                    className="mt-1 block w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 px-3 focus:outline-none dark:border-slate-800 dark:bg-slate-950 text-slate-900 dark:text-white font-mono font-bold tracking-wider"
                  />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Discount Type</label>
                  <select
                    value={formType}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Discount Value</label>
                  <input
                    type="number"
                    required
                    value={formValue}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Min Cart Amount (₹)</label>
                  <input
                    type="number"
                    value={formMinAmount}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Max Discount Limit (₹) (Optional)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Total Usage Limit</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Coupon Status</label>
                  <select
                    value={formStatus}
                ...
                <div className="sm:col-span-2 border-t border-slate-100 dark:border-slate-800 pt-3">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2 flex items-center gap-1"><Layers className="h-3.5 w-3.5" /> Promotion Target Rules</span>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Specific Category ID (Optional)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase">Specific Product ID (Optional)</label>
                  <input
                    type="text"
