...
export default function ProductsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-3xl flex items-center gap-2">
            Product Catalog <ShoppingBag className="h-6 w-6 text-slate-500" />
          </h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-dashed border-slate-500/30 bg-slate-500/5 px-3.5 py-2 text-xs font-bold text-slate-400 hover:bg-slate-500/10"
          >
            <Upload className="h-3.5 w-3.5" /> Excel Bulk Import
          </button>

          {/* Add product button */}
          <button
            onClick={handleOpenAdd}
            className="flex items-center gap-2 rounded-xl bg-slate-600 px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90 transition-opacity"
          >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 md:grid-cols-4 max-w-4xl text-sm">
        {/* Search */}
        <div className="flex items-center gap-2 bg-white dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-800 px-3 py-2 col-span-1 sm:col-span-2">
          ...
          <input
            ...
            className="w-full bg-transparent focus:outline-none dark:text-white text-slate-900"
          />
        </div>

        {/* Category filter */}
        <div className="flex items-center gap-2 bg-white dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-800 px-3 py-1.5">
          ...
          <table className="w-full text-left text-sm text-slate-500 dark:text-slate-400">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                return (
                  <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <img
                          ...
                          alt={p.name}
                          className="h-10 w-10 rounded-lg object-cover bg-slate-100 dark:bg-slate-950"
                        />
                        <div>
                          <span className="font-bold text-slate-900 dark:text-white block">{p.name}</span>
                          <span className="text-xs text-slate-400 font-mono">SKU: {p.variants?.[0]?.sku || 'No SKU'}</span>
                        </div>
                      </div>
                    </td>

                    <td className="px-6 py-4">
                      <span className="text-slate-800 dark:text-slate-300 block">{p.brand}</span>
                      <span className="text-xs text-slate-400 font-mono block">/{p.slug}</span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center gap-1 font-bold ${isLow ? 'text-red-500' : 'text-slate-700 dark:text-slate-300'}`}>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-1 flex-wrap">
                        {p.featured && <span className="bg-slate-500/10 text-slate-400 text-[9px] font-bold uppercase px-1 rounded">Feat</span>}
                        {p.trending && <span className="bg-amber-500/10 text-amber-400 text-[9px] font-bold uppercase px-1 rounded">Trend</span>}
                        {p.bestSeller && <span className="bg-emerald-500/10 text-emerald-400 text-[9px] font-bold uppercase px-1 rounded">Best</span>}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-slate-400 hover:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800"
              ...
              {filteredProducts.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-400">No products found matching query settings.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="relative w-full max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 text-sm">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 rounded-xl border border-slate-200 bg-slate-50 py-2 px-3 focus:outline-none dark:border-slate-800 dark:bg-slate-950 text-slate-900 dark:text-white"
                    />
                    <button
                      type="button"
                      onClick={addImage}
                      className="rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 dark:bg-slate-800 dark:text-slate-300"
                    >
                      Add Url
                    </button>
                  </div>
                      ...
                      {formImages.map((img, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-xs">
                          ...
                          <div className="flex items-center gap-1">
                            <button type="button" onClick={() => moveImage(idx, 'up')} disabled={idx === 0} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-30 text-white"><MoveUp className="h-3.5 w-3.5" /></button>
                            <button type="button" onClick={() => moveImage(idx, 'down')} disabled={idx === formImages.length - 1} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-30 text-white"><MoveDown className="h-3.5 w-3.5" /></button>
                    ...
                    <div>
                      ...
                      <div className="relative mt-1">
                        ...
                        <input
                          ...
                          className="block w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-3 focus:outline-none dark:border-slate-800 dark:bg-slate-950 text-slate-900 dark:text-white"
                        />
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-xl border border-slate-100 bg-slate-500/5 p-4 dark:border-slate-500/10 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2 text-slate-400">
                    <Calculator className="h-4 w-4" />
                    <span className="font-bold">Live Profit Estimation:</span>
                  </div>
                  <div className="flex gap-4">
                    <div className="text-white">Net Profit: <span className={`font-bold ${profitInfo.profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>₹{profitInfo.profit}</span></div>
                    <div className="text-white">Margin: <span className={`font-bold ${profitInfo.profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{profitInfo.margin.toFixed(1)}%</span></div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                  ...
                  <div className="rounded-xl border border-slate-100 dark:border-slate-800 p-3">
                    ...
                    <div className="flex gap-1 mt-1">
                      <input
                        ...
                        className="w-full rounded bg-slate-50 border border-slate-200 dark:border-slate-800 dark:bg-slate-950 py-1 px-2 text-xs text-slate-900 dark:text-white"
                      />
                      <button
                        type="button"
                        onClick={() => { if(newSize){ setSelectedSizes([...selectedSizes, newSize.toUpperCase()]); setNewSize(''); } }}
                        className="bg-slate-600 text-white rounded px-2.5 font-bold"
                      >
                        +
                      </button>
                    </div>
                ...
                {formVariants.length > 0 && (
                  <div className="border border-slate-100 dark:border-slate-800 rounded-xl p-4 bg-slate-950/10 space-y-2">
                      ...
                      {formVariants.map((v, index) => (
                        <div key={index} className="grid grid-cols-4 gap-2 items-center text-xs">
                          ...
                          <input
                            ...
                            className="rounded bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-2 py-1 text-slate-900 dark:text-white"
                          />
                          ...
                          <input
                            ...
                            className="rounded bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-2 py-1 text-slate-900 dark:text-white text-right font-bold"
                          />
                        </div>
                      ))}
                    </div>
      ...
      {excelModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm" onClick={() => setExcelModalOpen(false)} />
          ...
          <div className="relative w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
              <FileSpreadsheet className="h-5 w-5 text-emerald-500" /> Excel Spreadsheet Importer
            </h3>
            <p className="text-xs text-slate-400 mb-4 leading-relaxed">
              ...
              <code className="text-emerald-400 font-semibold font-mono">Product Name &emsp; Category &emsp; Regular Price &emsp; Selling Price &emsp; Stock &emsp; SKU</code>
            </p>
            ...
            <textarea
              ...
              rows={8}
              className="w-full font-mono text-xs p-3 rounded-xl border border-slate-200 bg-slate-50 focus:outline-none dark:border-slate-800 dark:bg-slate-950 text-slate-900 dark:text-white"
            />
