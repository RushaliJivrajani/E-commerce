    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground/80">Loading promotional banner files...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground dark:text-white sm:text-3xl flex items-center gap-2">
            Store Banners & Sliders <ImageIcon className="h-6 w-6 text-muted-foreground" />
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {banners.map((ban) => (
          <div
            key={ban.id}
            className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm dark:border-slate-800 dark:bg-foreground text-background/60 flex flex-col"
          >
            {/* Banner Preview Frame */}
            <div className="relative h-48 bg-card/80 dark:bg-slate-950 overflow-hidden shrink-0">
              ...
              <div className="absolute inset-0 bg-slate-950/40 p-4 flex flex-col justify-end text-white">
                ...
                <button type="button" className="mt-2 text-xs font-semibold bg-card text-foreground px-3 py-1 rounded w-max">{ban.buttonText}</button>
              </div>
            </div>
            ...
            <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
              <div className="text-xs space-y-1 text-muted-foreground/80">
                <div className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5 text-muted-foreground" /> Date Active: <span className="font-bold text-foreground dark:text-border">{ban.startDate} to {ban.endDate}</span></div>
                <div className="flex items-center gap-1.5"><Link2 className="h-3.5 w-3.5 text-muted-foreground" /> Redirects: <span className="font-bold text-foreground dark:text-border truncate">{ban.redirectLink}</span></div>
              </div>

              {/* Status and Action Buttons */}
              <div className="flex items-center justify-between border-t border-border/30 dark:border-slate-800 pt-3">
                <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${
                  ban.status === 'Active' ? 'bg-primary/100/10 text-primary' : 'bg-card/500/10 text-muted-foreground'
                }`}>{ban.status}</span>
                <div className="flex items-center gap-1">
                  <button onClick={() => handleOpenEdit(ban)} className="p-1.5 rounded-lg text-muted-foreground/80 hover:text-muted-foreground hover:bg-card/80 dark:hover:bg-slate-850"><Edit className="h-4 w-4" /></button>
                  <button onClick={() => handleDelete(ban.id)} className="p-1.5 rounded-lg text-muted-foreground/80 hover:text-rose-500 hover:bg-rose-500/10"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </div>
        ...
        {banners.length === 0 && (
          <p className="text-sm text-muted-foreground/80 text-center py-8 col-span-2">No promotional banners active. Click Add Banner above to display items.</p>
        )}
      </div>
          ...
          <div className="relative w-full max-w-xl rounded-2xl border border-border bg-card p-6 shadow-2xl dark:border-slate-800 dark:bg-foreground text-background">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted-foreground/80 uppercase">Banner Location Type</label>
                  <select
                    value={formType}
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted-foreground/80 uppercase">Banner Title</label>
                  <input
                    type="text"
                    required
                    value={formTitle}
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-muted-foreground/80 uppercase">Banner Image Link</label>
                  <input
                    type="text"
                    required
                    value={formImage}
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted-foreground/80 uppercase">Action Button Text</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted-foreground/80 uppercase">Redirect URL link</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted-foreground/80 uppercase">Start Date</label>
                  <input
                    type="date"
                    value={formStartDate}
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted-foreground/80 uppercase">End Date</label>
                  <input
                    type="date"
                    value={formEndDate}
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted-foreground/80 uppercase">Status</label>
                  <select
                    value={formStatus}
