...
const STATUS_CONFIG: Record<string, { color: string; bg: string; icon: React.ReactNode; label: string }> = {
  Placed:           { color: 'text-primary',  bg: 'bg-primary/10 border-primary/20',  icon: <Clock className="h-3.5 w-3.5" />,       label: 'Order Placed' },
  ...
  Delivered:        { color: 'text-primary',   bg: 'bg-primary/10/10 border-primary/30/20',     icon: <CheckCircle2 className="h-3.5 w-3.5" />, label: 'Delivered' },
  Cancelled:        { color: 'text-primary',     bg: 'bg-primary/10 border-primary/20',         icon: <XCircle className="h-3.5 w-3.5" />,      label: 'Cancelled' },
...
function OrderTimeline({ timeline, currentStatus }: { timeline: any[]; currentStatus: string }) {
  ...
  return (
    <div className="mt-4 border-t border-white/10 pt-4">
      <p className="text-[10px] font-bold text-primary uppercase tracking-wider mb-3">Tracking Timeline</p>
            ...
            return (
              <div key={st} className="flex flex-col items-center gap-1 flex-1">
                <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center transition-all ${
                    ...
                    ? 'bg-primary border-primary shadow-[0_0_10px_rgba(251,191,36,0.4)]'
                    : 'bg-foreground text-background border-white/20'
                } ${active ? 'scale-110' : ''}`}>
                  {done && <CheckCircle2 className="h-3 w-3 text-foreground" />}
                </div>
                <span className={`text-[9px] font-semibold text-center leading-tight hidden sm:block ${done ? 'text-primary' : 'text-muted-foreground'}`}>
        ...
        <div className="mt-4 space-y-2">
          {[...timeline].reverse().map((event, i) => (
            <div key={i} className="flex items-start gap-2.5 text-xs">
              <div className={`mt-0.5 h-2 w-2 rounded-full shrink-0 shadow-sm ${i === 0 ? 'bg-primary shadow-[0_0_5px_rgba(251,191,36,0.5)]' : 'bg-card/20'}`} />
              <div className="flex-1">
                <span className={`font-bold ${i === 0 ? 'text-primary' : 'text-muted-foreground/50'}`}>{event.status}</span>
...
function OrderCard({ order, onRefresh }: { order: any; onRefresh: () => void }) {
  ...
  return (
    <div className="glass-panel border border-white/10 rounded-2xl overflow-hidden shadow-xl hover:border-primary/30 transition-all">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 border-b border-white/10">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="text-lg font-black text-primary drop-shadow-md">₹{order.totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
          </div>
          <button
        ...
        {order.products?.map((prod: any, i: number) => (
          <div key={i} className="flex items-center gap-3 bg-card/5 rounded-xl p-3 border border-white/10 flex-1 min-w-[240px] hover:bg-card/10 transition-colors">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-bold text-primary mt-1">₹{prod.price?.toLocaleString('en-IN')} × {prod.quantity}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {expanded && (
        <div className="px-5 pb-5">
          ...
          <div className="mt-4 rounded-xl border border-white/10 p-4 space-y-2 text-xs bg-card/5">
            <p className="text-[10px] font-bold text-primary uppercase tracking-wider mb-2">Price Breakdown</p>
            ...
            {order.discountAmount > 0 && (
              <div className="flex justify-between text-primary font-bold bg-primary/10/10 p-1 px-2 rounded-lg -mx-2">
            ...
            <div className="flex justify-between text-muted-foreground/50">
              <span>Shipping</span>
              <span>{order.shippingCharges === 0 ? <span className="text-primary font-bold uppercase tracking-widest text-[10px]">Free</span> : `₹${order.shippingCharges}`}</span>
            </div>
            <div className="flex justify-between text-muted-foreground/50">
              <span>GST / Tax</span>
            ...
            <div className="flex justify-between font-black text-white border-t border-white/10 pt-2 mt-2">
              ...
              <span className="text-primary text-sm">₹{order.totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
            {canCancel && (
              <div className="pt-4 flex justify-end border-t border-white/10 mt-4">
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl border border-primary/50 bg-primary/10 px-4 py-2 text-xs font-bold text-primary hover:bg-primary hover:text-white disabled:opacity-50 transition-colors"
                >
    ...
    <div className="min-h-screen">
      ...
      <div className="glass-panel border-b border-white/10 shadow-sm relative z-10">
        <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <Link href="/" className="flex items-center gap-1 text-xs text-primary/80 hover:text-primary mb-2 transition-colors">
          ...
          {customer && (
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2.5 bg-card/5 border border-white/10 rounded-xl px-4 py-2.5">
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-foreground text-sm font-bold shrink-0 shadow-[0_0_10px_rgba(251,191,36,0.3)]">
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-primary/50 px-3 py-2.5 text-xs font-bold text-primary hover:bg-primary hover:text-white transition-colors"
              >
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 relative z-10">
        {orders.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center py-20 glass-panel border border-white/10 rounded-3xl mt-4">
            ...
            <Link
              href="/shop"
              className="inline-flex items-center gap-2 rounded-xl bg-primary px-6 py-3 text-sm font-bold text-foreground shadow-[0_0_20px_rgba(255,45,45,0.3)] hover:bg-primary transition-all uppercase tracking-widest"
            >
